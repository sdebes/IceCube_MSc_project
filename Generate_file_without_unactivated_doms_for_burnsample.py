...
One file of 100k events takes about 1.5-2 hours.
"""
...
rd_truth_path = "/groups/icecube/debes/storage/burnsample_predictions/14594307_stopped_muons_from_burnsample_with_pos_xyz_azimuth_zenith_reconstruction.parquet" #14.6m events from burnsample
mc_truth_weighted_path = "/groups/icecube/debes/storage/stopped_muons_final_reco_MC_GBweighted.csv" #8.3M events
savefolder = '/groups/icecube/debes/work/analyses/Efficiency/data/burnsample_reco_new_angle/'
...
mc_truth = pd.read_csv(mc_truth_weighted_path)
############################################
print('rd truth shape:', rd_truth.shape)
print('mc truth shape:', mc_truth.shape)


# Choose how many events to process
N_events = int(8000)
# Choose how many files the N_events events should be split into
N_files = 1
...
def find_photon_distance(DataFrame, truth = False):
    ...
    cherenkov_angle = (90-40.75) * np.pi / 180 # = arccos(1/index_of_refraction) # 90-40.75 because 40.75 is the angle of the cone
    ...
    start_points = end_points + 2000 * tracks # Largest distance between two DOMs in the data is 1557m 
    # Vectors going from the start points to the end points
    
    mu_vectors = end_points - start_points  # (R-Q)
    A_DOM_vector = end_points - dom_points

    projection_factor = np.sum(mu_vectors * A_DOM_vector, axis=1) / np.sum(mu_vectors ** 2, axis=1)
...
#    if iteration == 0:
#        print('Events', MIN, 'to', MAX, 'skipped.')
#        continue
    t0 = time.time()
    ...
    conn.close()
    print('rd_pulse shape:', rd_pulse.shape)
    ...
    print('rd_pulse shape after merge:', rd_pulse.shape)
    ...
    rd_part_done = find_photon_distance(rd)
    print('rd_part_done shape:', rd_part_done.shape)
    rd_part_done.to_parquet(savefolder + f'{N}k_rd_photon_reco_{iteration_string}.parquet')
    t1 = time.time()
    print('Data file completed in:', int((t001-t0)/60), 'minutes.')

    mc_part_done = find_photon_distance(mc)

    mc_part_done.to_parquet(savefolder + f'{N}k_mc_photon_reco_{iteration_string}.parquet')
    t2 = time.time()
    print('MC file completed in:', int((t2-t1)/60), 'minutes.')
    
    mc_truth_part_done = find_photon_distance(mc, truth=True)
    mc_truth_part_done.to_parquet(savefolder + f'{N}k_mc_truth_photon_reco_{iteration_string}.parquet')
    t3 = time.time()
    print('MC truth file completed in:', int((t3-t2)/60), 'minutes.')
    
    print('Data file has the shape', rd_part_done.shape)
    print('MC files have the shape', mc_part_done.shape)
